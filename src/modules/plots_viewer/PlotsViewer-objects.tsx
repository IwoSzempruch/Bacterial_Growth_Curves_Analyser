...
type CombinedLegend = {
  ...
  opacity: number
  size: { width: number; height: number } // aktualny rozmiar boxa (px)
  pos: { x: number; y: number }           // pozycja dla 'inside'
  chips: LegendChip[]                      // spłaszczone chipy (zachowana kolejność)
}
...
export default function PlotsViewer(){
  ...
  // Position of the overlay legend (when placement === 'inside'), relative to the chart container
  const [insideLegendPos, setInsideLegendPos] = useState<{ x: number; y: number }>({ x: 16, y: 16 });

  // Ref to the chart container; used as the positioning context for the inside legend
  ...
  const legendChips: LegendChip[] = useMemo(() => {
    ...
    for (const sampleName of orderedSamples) {
      ...
      for (const rep of reps) {
        ...
        const label = `R${rep.replicate} (${rep.well.toUpperCase()})`
        out.push({
          key,
          sample: sampleName,
          label,
  ...
  const seriesKeyByName = useMemo(() => {
    ...
    legendChips.forEach(ch => map.set(ch.seriesName, ch.key))
    return map
  }, [legendChips])
  ...
  // Rozmiar legendy zależny od trybu (px). Startowe minimum bazujące na heurystyce.
  const [legendBoxSize, setLegendBoxSize] = useState<{ right:{w:number,h:number}, below:{w:number,h:number}, inside:{w:number,h:number} }>({
    right:  { w: 320, h: 240 },
    below:  { w: 600, h: 140 },
    inside: { w: 280, h: 160 },
  })

  // Przezroczystość legendy – we wszystkich trybach
  const [legendOpacity, setLegendOpacity] = useState<number>(0.9)

  // Pozycja dla 'inside' (utrzymujemy istniejący stan)
  useEffect(()=>{ setInsideLegendPos(p=>p) }, []) // placeholder
  ...
  const legendSections = useMemo(() => {
    // grupowanie chipów po sample i filtrowanie po selectedKeys
    const visible = legendChips.filter(ch => selectedKeys.includes(ch.key))
    const bySample = new Map<string, LegendChip[]>()

    for (const ch of visible) {
      if (!bySample.has(ch.sample)) bySample.set(ch.sample, [])
      bySample.get(ch.sample)!.push(ch)
    }

    // zachowaj kolejność próbek (orderedSamples)
    return orderedSamples
      .filter(s => bySample.has(s))
      .map(sample => ({
        sample,
        replicates: bySample.get(sample)!.sort((a, b) => a.order - b.order)
          .map(ch => ({ key: ch.key, label: ch.label, color: ch.color }))
      }))
  }, [legendChips, orderedSamples, selectedKeys])
  ...
  const renderLegend = useCallback(
    (placement: LegendPlacement) => {
      ...
      function LegendContent() {
        ...
        // FLEX LAYOUT – zgodnie z wymaganiem
        ...
        // Build chips (zmienna szerokość, rosną jak flex-grow:1)
        ...
        const setBoxSizeLocal = (next: { w?: number; h?: number }) => {
          setLegendBoxSize(prev => {
            ...
            const h = Math.max(100, next.h ?? cur.h)
            if (legendPlacement === 'right') return { ...prev, right: { w, h } }
            if (legendPlacement === 'below') return { ...prev, below: { w, h } }
        ...
        const onResizeBottomMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
          ...
          const onMove = (ev: MouseEvent) => setBoxSizeLocal({ h: startH + (ev.clientY - startY) })
          const onUp = () => { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp) }
          window.addEventListener('mousemove', onMove); window.addEventListener('mouseup', onUp)
        }
              ...
              {chips.map(({ section }) => (
                <div
                  ...
                  style={{
                    ...
                    whiteSpace: 'nowrap',
                    flex: '1 1 220px', // efekt flex-grow:1
                  }}
                >
    ...
    [legendSections, legendChipMetrics, legendVisible, legendTitle, legendContentScale, legendOpacity, legendPlacement, legendBoxSize, insideLegendPos],
  );
  ...
  // Heuristic widths (dla wstępnego układu; realny rozmiar bierze legendBoxSize)
  const minNatural = legendChipMetrics.length
    ? Math.min(...legendChipMetrics.map(m => m.naturalWidth))
    : 220;

  // const sideLegendWidth = ...
  // const sideLegendMinWidth = ...
  // const insideLegendMinWidth = ...
  // (nieużywane — rozmiar bierzemy z legendBoxSize)
        ...
        <>
          ...
          <div className="panel" style={{ marginTop: 12 }}>
            ...
            <div className="row" style={{ justifyContent: 'space-between', gap: 8, marginTop: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {legendVisible && (
                <div className="row" style={{ gap: 6, alignItems: 'center' }}>
                  <span className="small">Legend opacity:</span>
                  <input
                    type="range"
                    min={0.2}
                    max={1}
                    step={0.05}
                    value={legendOpacity}
                    onChange={(e) => setLegendOpacity(+e.target.value)}
                    style={{ width: 120 }}
                  />
                  <span className="small">{Math.round(legendOpacity * 100)}%</span>
                </div>
              )}
            </div>

            <div style={{ marginTop: 12 }}>
              <div ref={chartRef} style={legendPlacement === 'right' && legendVisible ? { display:'flex', gap:12, alignItems:'stretch', flexWrap:'wrap' } : undefined}>
                <div style={legendPlacement === 'right' && legendVisible ? { flex:'1 1 0', minWidth:320, position:'relative' } : { position:'relative' }}>
                  ...
                  {legendPlacement === 'inside' && legendVisible && (
                    <div style={{
                      position:'absolute',
                      left: insideLegendPos.x,
                      top: insideLegendPos.y,
                      width: legendBoxSize.inside.w,
                      height: legendBoxSize.inside.h,
                      zIndex: 2,
                      overflow: 'hidden',
                    }}>
                      {renderLegend('inside')}
                ...
                {legendPlacement === 'right' && legendVisible && (
                  <div style={{ flex:`0 0 ${legendBoxSize.right.w}px`, minWidth:legendBoxSize.right.w, maxWidth:'50%' }}>
                    {renderLegend('right')}
                  </div>
                )}
              </div>
              ...
              {legendPlacement === 'below' && legendVisible && (
                <div style={{ marginTop: 10, display:'flex', justifyContent:'center' }}>
                  <div style={{ width:'100%', maxWidth:`calc(100% - ${AXIS_HORIZONTAL_PADDING}px)`, height: legendBoxSize.below.h }}>
                    {renderLegend('below')}
                  </div>
                </div>
              )}
