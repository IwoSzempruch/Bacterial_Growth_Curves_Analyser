...
function normalizeAssignments(map: Record<string, string>): Record<string, string> {
  const out: Record<string, string> = {}
  Object.entries(map ?? {}).forEach(([well, sample]) => {
    const normalizedWell = formatWellA01(String(well))
    const trimmedSample = String(sample ?? '').trim()
    if (!normalizedWell || !trimmedSample) return
    out[normalizedWell] = trimmedSample
  })
  return out
}
...
export function buildAssignmentEntry(dataset: UnifiedDataset, mapping: Mapping): AssignmentEntry | null {
  ...
  const wellsInDataset = Array.from(
    new Set(
      dataset.rows
        .filter((r) => Number.isFinite(r?.timeSeconds) && Number.isFinite(r?.value))
        .map((r) => formatWellA01(r.well)),
    ),
  ).sort((a, b) => String(a).localeCompare(String(b), undefined, { numeric: true }))

  const sampleToWells: Record<string, string[]> = {}
  const mappingAssignments = normalizeAssignments(
    Object.keys(mapping.assignments ?? {}).length > 0
      ? mapping.assignments
      : Object.fromEntries(
          (Array.isArray((mapping as any).samples) ? (mapping as any).samples : [])
            .flatMap((s: any) =>
              Array.isArray(s?.wells)
                ? s.wells.map((w: any) => [formatWellA01(w), s?.name ?? ''] as const)
                : [],
            )
            .filter(([w, name]: [string | null, string]) => Boolean(w && name)),
        ),
  )
  wellsInDataset.forEach((well) => {
    const sample = (mappingAssignments[well] ?? '').trim()
    if (!sample) return
    if (!sampleToWells[sample]) sampleToWells[sample] = []
    sampleToWells[sample].push(formatWellA01(well))
  })
  const mappingWellCount = Object.keys(mappingAssignments).length
  const datasetWellCount = wellsInDataset.length
  if (mappingWellCount !== datasetWellCount) {
    const missingInData = Object.values(sampleToWells)
      .flat()
      .filter((w) => !wellsInDataset.includes(w))
      .slice(0, 12)
    const extraInData = wellsInDataset.filter(
      (w) => !Object.values(sampleToWells).flat().includes(w),
    ).slice(0, 12)
    // Log a concise, developer-friendly message; UI stays untouched.
    console.warn(
      `[ASSIGNMENT] Well count mismatch for run "${dataset.sourceFile || dataset.runId}": ` +
        `mapping has ${mappingWellCount} wells, dataset has ${datasetWellCount}. ` +
        (missingInData.length ? `Missing in data: ${missingInData.join(', ')}. ` : '') +
        (extraInData.length ? `Extra in data: ${extraInData.join(', ')}. ` : ''),
    )
  }
  ...
  dataset.rows.forEach((row) => {
    ...
    const sample = (mappingAssignments[formattedWell] ?? '').trim()
    if (!sample) return
