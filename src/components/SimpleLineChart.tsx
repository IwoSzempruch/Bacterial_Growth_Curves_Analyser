...
export default function SimpleLineChart({
  ...
  bands?: { name: string; color: string; points: { x: number; low: number; high: number }[] }[]
  ...
  const availableHeight =
    containerSize.height > 0
      ? Math.max(0, containerSize.height - canvasPaddingAllowance)
  ...
  const computedH = hasAvailableHeight
    ...
    : Math.max(minHeight, Math.min(maxHeight, Math.min(byAspect, byViewport)))
  ...
  useEffect(()=>{
    ...
    return ()=>{
      c?.removeEventListener('wheel', handler as any)
      s?.removeEventListener('wheel', handler as any)
    }
  ...
  return (
    <div ref={containerRef} style={{ position:'relative', width: '100%', maxWidth: '100%', overflowX: 'hidden', overscrollBehavior: 'none' }}>
      <svg width={width} height={computedH} role="img" ref={(el)=>{ svgRef.current = el; onSvgRef && onSvgRef(el) }} onDoubleClick={enableDoubleClickReset ? onDoubleClick : undefined} onMouseDown={onMouseDown} onMouseMove={(e)=>{ onMouseMove(e); updateHover(e.clientX, e.clientY); }} onMouseUp={onMouseUp} onMouseLeave={()=>{ cancelSelection(); setHover(null); dragRef.current = null; setPanning(false); selectionModeRef.current = 'none'; }}>
        ...
        <g clipPath="url(#plot-clip)">
        {mode === 'line' && stdMode !== 'none' && bands && bands.map((b, idx) => {
