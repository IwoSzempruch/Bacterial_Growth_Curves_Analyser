...
DEFAULT_OD_MIN = 0.01        # minimalny OD po blankowaniu, żeby w ogóle brać punkt pod uwagę
DEFAULT_FRAC_K_MAX = 0.4     # maksymalna frakcja plateau (K_est), przy której uznajemy, że jeszcze nie ma fazy stacjonarnej
...
def get_dataset_rows(data):
    """Zwraca dataset['rows'], niezależnie od tego, czy dataset jest w root, czy wewnątrz assignments."""
...
def list_samples_and_wells_from_mapping(data):
    """Zwraca mapę sample -> posortowana lista wells (na podstawie mappingu lub dataset.rows)."""
...
def get_time_series_for_well(data, well: str):
    """Zwraca (t, y, sample_name, replicates) dla pojedynczego wella."""
    rows = get_dataset_rows(data)
    t = []
    y = []
    sample_name = None
...
def detect_log_phase(
    ...
    Założenia:
    - dane są już oczyszczone ze spike'ów,
    - (opcjonalnie) po blankowaniu,
    - wzrost w początkowej fazie jest monotonicznie niemalejący.
...
def plot_log_phase(t, y, log_indices, mu_max, mu_mean, K_est, title=""):
    ...
    if mu_max is not None:
        legend_text.append(f"µ_max ~ {mu_max:.4f} (1/min)")
    if mu_mean is not None:
        legend_text.append(f"µ_mean (log) ~ {mu_mean:.4f}")
    if legend_text:
        plt.suptitle(" | ".join(legend_text), fontsize=9, y=0.98)
...
class LogPhaseApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Log phase detection tester")
    ...
    def _build_ui(self):
        ...
        # --- wybór sample / well ---
        selection_frame = ttk.LabelFrame(self.root, text="Wybór sample / well")
    ...
    def _load_from_entry(self):
        path = self.file_var.get().strip()
        if not path:
            messagebox.showinfo("Brak pliku", "Podaj ścieżkę do pliku JSON.")
            return
        try:
            self._load_json(path)
        except Exception as exc:  # noqa: BLE001
    ...
    def _run_analysis(self):
        ...
        try:
            ...
            messagebox.showerror("Błąd parametrów", "Nieprawidłowe wartości parametrów (window_size, R², OD itd.).")
            return
